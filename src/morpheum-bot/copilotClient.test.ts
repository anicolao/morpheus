...
describe('CopilotClient', () => {
  ...
  it('should include issue creation and session tracking in streaming updates', async () => {
    ...
    expect(chunks.some(chunk => chunk.includes('https://github.com/owner/repo/issues/123'))).toBe(true);
    expect(chunks.some(chunk => chunk.includes('Starting GitHub Copilot session for issue #123'))).toBe(true);
