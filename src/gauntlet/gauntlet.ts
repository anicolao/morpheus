...
interface GauntletTask {
  id: string;
  skill: 'Environment Management & Tooling' | 'Software Development & Refinement';
...
const tasks: GauntletTask[] = [
  {
    ...
    successCondition: async (containerName) => {
      const { stdout } = await execa('nix', ['develop', '-c', 'docker', 'exec', containerName, 'which', 'jq'], { cwd: './jail' });
  ...
  },
  {
    ...
    successCondition: async (containerName) => {
      const { stdout } = await execa('nix', ['develop', '-c', 'docker', 'exec', containerName, 'which', 'xmlstarlet'], { cwd: './jail' });
      return stdout.includes('/nix/store');
    },
  },
  {
    ...
    skill: 'Environment Management & Tooling',
    difficulty: 'Hard',
    prompt: "A Python script with 'import requests' is failing. Fix the environment so it can run successfully.",
    successCondition: async (containerName) => {
      const { stdout } = await execa('nix', ['develop', '-c', 'docker', 'exec', containerName, 'python', '-c', 'import requests'], { cwd: './jail' });
      return stdout === '';
    },
  },
  {
    ...
    successCondition: async (containerName) => {
      const { stdout } = await execa('nix', ['develop', '-c', 'docker', 'exec', containerName, 'cat', 'server.js'], { cwd: './jail' });
  ...
  },
  {
    ...
    successCondition: async (containerName) => {
      const { stdout } = await execa('nix', ['develop', '-c', 'docker', 'exec', containerName, 'ls'], { cwd: './jail' });
...
async function evaluateSuccessCondition(task: GauntletTask, containerName: string, verbose: boolean): Promise<boolean> {
  try {
    return await task.successCondition(containerName);
  } catch (error) {
    if (verbose) {
      console.error(`Error evaluating success condition for task ${task.id}:`, error);
    } else {
      console.error(`Task ${task.id} failed: Success condition check threw an error.`);
    }
    return false;
  }
}
...
async function runGauntlet(model: string, taskId: string, results: GauntletResult, verbose: boolean) {
  ...
  try {
    const { stdout } = await execa('nix', ['develop', '-c', 'docker', 'ps', '-a', '--filter', 'name=gauntlet-test-', '--format', '{{.Names}}'], { cwd: "./jail", timeout: 5000 });
    const containers = stdout.split('\n').filter(line => line.startsWith('gauntlet-test-'));
    if (containers.length > 0) {
      console.log(`Stopping ${containers.length} previous test containers...`);
      try {
        await execa('nix', ['develop', '-c', 'docker', 'stop', ...containers], { cwd: "./jail", timeout: 5000 });

        // Verify that all gauntlet containers are gone
        const { stdout: stoppedCheck } = await execa('nix', ['develop', '-c', 'docker', 'ps', '-a', '--filter', 'name=gauntlet-test-', '--format', '{{.Names}}'], { cwd: "./jail", timeout: 5000 });
        const stoppedContainers = stoppedCheck.split('\n').filter(line => line.startsWith('gauntlet-test-'));
        if (stoppedContainers.length > 0) {
          console.warn(`Warning: ${stoppedContainers.length} gauntlet containers were not removed and need manual cleanup:`, stoppedContainers);
        } else {
          console.log("Successfully stopped and removed all previous gauntlet containers.");
        }
      } catch (error) {
    ...
    console.error("Error stopping previous test containers. Is docker running?", error);
  }
  ...
  const containerName = await bot.processMessage(`!create ${port}`, "gauntlet", messageSender);
  if (!containerName) {
  ...
  // 3. Run the task
  const result = await bot.processMessage(task.prompt, "gauntlet", messageSender);
