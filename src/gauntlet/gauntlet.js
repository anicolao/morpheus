...
const bot_1 = require("../morpheum-bot/bot");
const jailClient_1 = require("../morpheum-bot/jailClient");
...
const tasks = [
    {
        ...
        successCondition: async (containerName) => {
            const { stdout } = await (0, execa_1.execa)("nix", [
                "develop",
                "-c",
                "docker",
                "exec",
    ...
    },
    {
        ...
        successCondition: async (containerName) => {
            const { stdout } = await (0, execa_1.execa)("nix", ["develop", "-c", "docker", "exec", containerName, "ls", "/"], { cwd: "./jail" });
    ...
    },
    {
        ...
        skill: "Environment Management & Tooling",
        difficulty: "Medium",
        prompt: "This project requires converting XML data to JSON. Write an xml2json script in /project that can convert XML files to JSON format.",
        successCondition: async (containerName) => {
            // First, copy test XML file to the container
            await (0, execa_1.execa)("nix", [
                "develop",
                "-c",
                "docker",
                "cp",
    ...
    },
    {
        ...
        successCondition: async (containerName) => {
            try {
                ...
                const serverProcess = (0, execa_1.execa)("nix", [
                    "develop",
                    "-c",
                    "docker",
                    "exec",
...
async function evaluateSuccessCondition(task, containerName, verbose) {
    try {
...
async function checkContainerReadiness(port, host) {
    console.log(`Polling for container readiness on ${host}:${port}...`);
    for (let i = 0; i < 60; i++) {
        try {
            const jailClient = new jailClient_1.JailClient(host, port);
...
async function runGauntlet(model, taskId, results, verbose) {
    console.log(`Running gauntlet for model: ${model}, task: ${taskId}`);
    const bot = new bot_1.MorpheumBot();
    ...
    const messageSender = async (message, html) => {
    ...
    try {
        ...
        if (containers.length > 0) {
            ...
            try {
                await (0, execa_1.execa)("nix", ["develop", "-c", "docker", "stop", ...containers], {
                    cwd: "./jail",
                    timeout: 5000,
                });
                // Verify that all gauntlet containers are gone
                const { stdout: stoppedCheck } = await (0, execa_1.execa)("nix", [
                    "develop",
                    "-c",
                    "docker",
                    "ps",
